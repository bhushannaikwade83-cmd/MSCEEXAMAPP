...
class WebPinLoginScreen extends StatefulWidget {
  const WebPinLoginScreen({super.key});

  @override
  State<WebPinLoginScreen> createState() => _WebPinLoginScreenState();
}

class _WebPinLoginScreenState extends State<WebPinLoginScreen> {
  ...
  Future<void> _loadSavedPin() async {
    ...
    debugPrint('🔐 DEBUG: Loaded PIN from SessionService: $pin');
    if (mounted) {
      setState(() {
        _savedPin = pin;
        _centreId = centre?['id'];
  ...
  Future<void> _verifyPin() async {
    ...
    debugPrint('🔐 DEBUG: Verifying PIN - Entered: $enteredPin, Saved: $_savedPin, Match: ${enteredPin == _savedPin}');

    if (enteredPin == _savedPin) {
      debugPrint('✅ DEBUG: PIN match verified!');
                        ...
                        onPressed: () {
                          showDialog(
                            ...
                            builder: (ctx) => AlertDialog(
                              ...
                              content: const Text(
                                'You will need to enter username and password for a different centre.',
                              ),
                              actions: [
                                TextButton(
                                  ...
                                  onPressed: () async {
                                    ...
                                    if (mounted) {
                                      ...
                                      Navigator.pushAndRemoveUntil(
                                        context,
                                        MaterialPageRoute(
                                          builder: (_) => const WebCenterLoginScreen(),
                                        ),
                                        (_) => false,
                                      );
                                    }
